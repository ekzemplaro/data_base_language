...
rm -f $WORK_FILE_AA
#
awk -F "." '{print "grep " $1 " */*.pike"}' $REF_HOME"/common.txt" > $WORK_FILE_AA
#
bash $WORK_FILE_AA > $WORK_FILE_BB
$BIN/pick_up.js $REF_HOME"/common.txt" $WORK_FILE_BB  > $WORK_FILE_CC
#
awk '{print $1".pike"}' $WORK_FILE_CC > reference.txt
#
#
#
awk -F "." '{print "grep " $1 " */*.pike"}' $REF_HOME"/common.txt" >> /tmp/t01
#
