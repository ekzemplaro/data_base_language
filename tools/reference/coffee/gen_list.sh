...
rm -f $WORK_FILE_AA
#
awk -F "." '{print "grep " $1 " */*.coffee"}' $REF_HOME"/common.txt" > $WORK_FILE_AA
#
bash $WORK_FILE_AA | egrep 'require' > $WORK_FILE_BB
$BIN/pick_up.js $REF_HOME"/common.txt" $WORK_FILE_BB  > $WORK_FILE_CC
#
awk '{print $1".coffee"}' $WORK_FILE_CC > reference.txt
#
#
#
awk -F "." '{print "grep " $1 " */*.coffee"}' $REF_HOME"/common.txt" >> /tmp/t01
#
