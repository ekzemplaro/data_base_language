...
// --------------------------------------------------------------
public class db2_java_read extends HttpServlet 
{
// --------------------------------------------------------------
public void doGet
		...
		throws IOException,ServletException
{
	...
	try
	{
out.println ("*** aaaa Mar/17/2000 *** doGet ***");


out.println ("*** bbbb *** doGet ***");
	Class.forName ("COM.ibm.db2.jdbc.app.DB2Driver").newInstance();

out.println ("*** cccc *** doGet ***");
	}
	catch (Exception ee)
		{
		ee.printStackTrace ();
		out.println ("*** error *** Class.forName ***");
		out.println ("*** error *** db2_java_read.doGet ***");
...
// --------------------------------------------------------------
void db2_disp_proc (PrintWriter out)
{
out.println ("*** start *** db2_java_read.db2_disp_proc ***");
...
out.println ("*** bbbb *** db2_java_read.db2_disp_proc ***");

	Connection conn = DriverManager.getConnection (url);

out.println ("*** cccc *** db2_java_read.db2_disp_proc ***");

	sql_to_json.sql_to_json_proc (conn,out);

out.println ("*** dddd *** db2_java_read.db2_disp_proc ***");
	}
	catch (Exception ee)
		{
		ee.printStackTrace ();
		out.println
			("*** error *** db2_java_read.db2_disp_proc ***");
		out.println ("\turl = " + url);
		out.println ("*** ee *** " + ee.toString ());
		}

out.println ("*** end *** db2_disp_proc ***");
}
